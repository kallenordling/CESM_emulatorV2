    ...
    for i in tqdm(range(config.samples_per)):
        ...
        for tensor_batch, coords in tqdm(
            dataloader, disable=not accelerator.is_main_process
               ...
               cond_batch = []
               for c in coords:
                   cond_batch.append(dataset.get_cond_from_coords(c))
               cond_batch = torch.stack(cond_batch).to(accelerator.device)

               gen_months = generate_samples2(
                   clean_samples=tensor_batch,   # or some other reference field
                   cond_map=cond_batch,
                   scheduler=scheduler,
